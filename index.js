...
function createItemCard(id, preview, name, brand, price) {
  ...
  var newPriceText = document.createTextNode("₹ ");
...
function getProductDetail() {
  ...
  // Get Product Details from items.json
  $.getJSON("items.json", function(data) {
    var productDetail = data.find(item => item.id === searchId);
    ...
    if (productDetail) {
      ...
      console.error("Product not found in items.json");
    }
  });
}
getProductDetail();
